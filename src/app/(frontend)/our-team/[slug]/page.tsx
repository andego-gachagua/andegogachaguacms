...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!team_member) {
    return {
      title: 'Team Member Not Found - Andego Gachagua Advocates LLP',
      description:
        'The team member you are looking for could not be found. Meet the rest of our dedicated legal professionals at Andego Gachagua Advocates LLP.',
    }
  }

  const memberName = team_member.name || 'Our Team - Andego Gachagua Advocates LLP'
  const memberRole = team_member.role || 'Team Member at Andego Gachagua Advocates LLP'
  const memberBioExcerpt =
    (Array.isArray(team_member.bio) && team_member.bio[0]?.children?.[0]?.text?.slice(0, 150)) ||
    'Discover our team of dedicated legal professionals at Andego Gachagua Advocates LLP.'

  return {
    title: `${memberName} - ${memberRole} | Andego Gachagua Advocates LLP`,
