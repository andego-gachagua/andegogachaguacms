...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const [teamRes, postsRes, areaRes] = await Promise.all([
    fetch(`${process.env.NEXT_PUBLIC_PAYLOAD_API_URL}/team`),
    fetch(`${process.env.NEXT_PUBLIC_PAYLOAD_API_URL}/publications`, {
      next: { revalidate: 3600 },
    }),
    fetch(`${process.env.NEXT_PUBLIC_PAYLOAD_API_URL}/practice-areas`),
  ]);
